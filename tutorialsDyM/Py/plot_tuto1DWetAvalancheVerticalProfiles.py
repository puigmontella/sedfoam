...
p_rbgh_0 = fluidfoam.readscalar(sol, '200', 'p_rgh')
Ua_0= fluidfoam.readvector(sol,  '200', 'U')
...
times=[10,20,60,100,200]# please select specific times where data will be reconstructed
...
for i in range(len(times)):
	...
	Ua_A = fluidfoam.readvector(sol, tread, 'U')
	pff_A = fluidfoam.readscalar(sol, tread, 'pff')
	pa_A = fluidfoam.readscalar(sol, tread, 'pa')
	p_rbgh_A = fluidfoam.readscalar(sol, tread, 'p_rgh')
	vel_values=[]
	phi_values=[]
	y_values=[]
	p_particle_values=[]
