...
U = fluidfoam.readvector(sol, tread, 'U')
...
try:
    gradU = fluidfoam.readtensor(sol, tread, 'grad(U)')
except:
    print("grad(U) was not found -> postProcess -func 'grad(U)'")
    os.system("postProcess -case "+sol+" -func \'grad(U)\' -time "+tread)
    gradU = fluidfoam.readtensor(sol, tread, 'grad(U)')
...
ax2 = subplot(gs[0, 1])
l21, = ax2.plot(U[0,:]/(g*d)**0.5, Y[:]/d, '-r')
...
ax2.set_xlim(0,  np.max(uex)/(g*d)**0.5 * 1.1)
