...
proc = subprocess.Popen(
	['foamListTimes', '-latestTime', '-case', solpath], stdout=subprocess.PIPE)

output = proc.stdout.read()
tread = output.decode().rstrip().split('\n')[0]

treadd=np.arange(10,int(tread),10)
print (treadd)
for kk in range(len(treadd)):
	
	Nt, y, u, alpha0, Tausxy,Taufxy  = readOpenFoam(solpath,str(treadd[kk]))


	print ('total mass:',sum(alpha0))
	#
	# Plots
	#
	...
	#
	ax1 = subplot(gs[0, 0])
	p1_4 = ax1.plot(u, (y + Z0num) / (2. * rs), '-r')
	...
	p3_2 = ax3.plot(Taufxy, (y + Z0num) / (2. * rs), '--b')
...
#	p4_2 = ax4.plot( k, (y + Z0num) / (2. * rs), '-b',label='TKE: k')
	#p4_2 = ax4.plot(1.5*Theta, (y + Z0num) / (2. * rs), '--r', label=r'Gran. Temp.: $3/2\theta$')
	...
	savefig('Figures/video/res1_tuto3muI_%s.png'%kk, facecolor='w', edgecolor='w', format='png')
	ax1.clear()
	ax2.clear()
	ax3.clear()
	ax4.clear()
