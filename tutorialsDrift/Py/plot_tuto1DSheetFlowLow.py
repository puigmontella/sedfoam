...
def readOpenFoam(sol):
	...
	Tau = fluidfoam.readtensor(sol, tread, 'Tau')

	k = fluidfoam.readscalar(sol, tread, 'k')
	# Theta = fluidfoam.readscalar(sol, tread, 'Theta')
	return Nt, Y, Ua[0, :], Ub[0, :], alpha, Tauf[3, :], Taus[3, :], Tau[3, :], k
...
Nt, y, ua, ub, alpha0, Taufxy, Tausxy,Tauxy, k = readOpenFoam(solpath)
...
#
ax3 = subplot(gs[0, 2])
p3_2 = ax3.plot( Tauxy, (y + Z0num) / (2. * rs), '-g')
...
#
ax4 = subplot(gs[0, 3])
p4_2 = ax4.plot( k, (y + Z0num) / (2. * rs), '-g',label='TKE: k')
...
savefig('Figures/res1_tuto3muIv2.png', facecolor='w', edgecolor='w', format='png')
